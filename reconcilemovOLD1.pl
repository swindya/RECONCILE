...
for ($p = 0; $p <= $ijmlbaris; $p++) {
	if (($inetset[$i] == 'VAULT') || ($inetset[$i] == 'VAULT1')) {
		$istatmatch[$p] = 0;
	}
	elsif ($inetset[$i] == 'BIS4') {
		$istatmatch[$p] = 15;
	}
	else {
		$istatmatch[$p] = 0;
...
if (($stafile[1] == 1) && ($stafile[2] == 1)) {
	...
	for (my $i = 0; $i <= $ijmlbaris; $i++) {
		for (my $ii = 0; $ii <= $e1jmlbaris; $ii++) {
			$iistr = '';
			if (($e1statmatch[$ii] == 0) && ($istatmatch[$ii] == 0)) {
					...
					if ($e1setdate[$ii] eq $isetdate[$i]) {
						...
						if (lc($e1acc[$ii]) eq lc($iaccdepo[$i])) {
							...
							if (lc($iordertype[$i]) eq lc('Delivery Versus Payment')) {
								...
								if (((lc($e1scode[$ii]) eq lc($iscode[$i])) && ($e1qty[$ii] == $iqty[$i])) || ((lc($e1netsac[$ii]) eq lc($inetsac[$i])) && (lc($e1netsa[$ii]) eq lc($inetsa[$i])))) {
									$e1statmatch[$ii] = 9;
									$istatmatch[$i] = 9;
...
#print "<ul></ul>" . "\n";
print "Jumlah data INTRL   yg NO MATCH : " . ($ijmlbaris-$ijmlku) . "/" . $ijmlbaris . "<br>\n";
if (($stafile[1] == 1) && ($stafile[2] == 1)) {
